...
const StateSelector: React.FC = () => {
  ...
  const renderStateButton = (state: ToothState) => {
    ...
    return <button key={state} onClick={() => setSelectedState(state)} className={cn("flex items-center space-x-2 p-2 rounded-lg border-2 transition-all duration-200", "hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500", isSelected ? "border-blue-500 bg-blue-50 shadow-md scale-105" : "border-gray-200 hover:border-gray-300")}>
        {/* Indicador de color o símbolo */}
        <div className={cn("w-4 h-4 rounded border flex items-center justify-center text-xs font-bold", hasSymbol ? "border-gray-300 bg-white" : `${config.bg} ${config.border}`)} style={hasSymbol ? {
        color: getSymbolColor(state)
      } : {}}>
          {hasSymbol && symbol}
        </div>
        
        {/* Etiqueta */}
        <span className={cn("text-xs font-medium", isSelected ? "text-blue-700" : "text-gray-700")}>
          {config.label}
        </span>
      </button>;
  };
